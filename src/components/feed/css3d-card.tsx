...
export function CSS3DCard({ post, size = "standard", stagger = 0, index = 0 }: CSS3DCardProps) {
  const { t } = useI18n();
  const section = SECTIONS[post.sectionId];

  return (
    <div
      className={`group reveal-in stagger-${Math.min(index % 9 + 1, 9)}`}
      style={{
    ...
    >
      <Link
        href={`/post/${post.id}`}
        className="card-shell card-shine block rounded-xl overflow-hidden"
        style={{
          boxShadow: `0 4px 16px rgba(0,0,0,0.2), 0 0 0 1px rgba(232,233,237,0.05)`,
        }}
      >
